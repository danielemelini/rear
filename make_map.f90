...
!    - 
!
! Copyright (C) Giorgio Spada (UNIURB) and Daniele Melini (INGV), 2013
!
! History: 
...
! ------------------------------------------------------------- 
!
 open(20,file=file_ice,status='unknown')
!
...
!
 rewind(20)
!
 allocate( lon_ice(nice), lat_ice(nice), h1(nice), alfa_ice(nice) )  
!
...
!--------------------------------------------------------------------
!
 Open(30,file=file_gf,status='unknown') 
!
...
! --------------------------------------------------------------------------
!
 open(30,file=file_in_points,status='unknown') 
!
!
...
!$OMP PARALLEL DO &
!$OMP    DEFAULT(NONE) &
!$OMP    PRIVATE(I,J,COSTHETA,K,IJ,THETA,XI,DELTA,THETA_STEP,UR_D,UH_D,N_D,COSGAMMA,SINGAMMA,UTH_D,UPHI_D) &
!$OMP   SHARED(IN_GR,NICE,SIN_LAT_ICE,COS_LAT_ICE,SIN_LON_ICE,COS_LON_ICE, &
!$OMP          SIN_LAT_OBS,SIN_LON_OBS,COS_LAT_OBS,COS_LON_OBS, &
!$OMP          T1,T2,NSEC,NGRID,THETA_GRID,UR_GRID,N_GRID,UH_GRID,Q,R,H1, &
...
!
        u_obs(i) =    u_obs(i)    + ur_d 
        uth_obs(i)  = uth_obs(i)  + uth_d 
        uphi_obs(i) = uphi_obs(i) + uphi_d 
        n_obs(i)    = n_obs(i)    + n_d 
!
     end do loop_ice 
! 
